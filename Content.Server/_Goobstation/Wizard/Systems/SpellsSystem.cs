...
public sealed class SpellsSystem : SharedSpellsSystem
{
    ...
    private void OnMonkeyAscension(Entity<MindContainerComponent> ent, ref SummonSimiansMaxedOutEvent args)
    {
        ...
        foreach (var (action, _) in Actions.GetActions(uid))
        {
            ...
            if (TryComp(action, out StoreRefundComponent? refund) && refund.StoreEntity.HasValue &&
                TryComp<StoreComponent>(refund.StoreEntity.Value, out var storeComp))
                _store.DisableRefund(refund.StoreEntity.Value, storeComp);
    ...
    protected override void ShootSpellCards(SpellCardsEvent ev, EntProtoId proto)
    {
        ...
        for (var i = 0; i < ev.ProjectilesAmount; i++)
        {
            ...
            {
                _gun.SetTarget(newUid, ev.Entity.Value, out var targeted, false);
                Entity<SpellCardComponent, PhysicsComponent, TargetedProjectileComponent> ent = (newUid, spellCard,
                    physics,
                    targeted);
                Dirty(ent);
            }

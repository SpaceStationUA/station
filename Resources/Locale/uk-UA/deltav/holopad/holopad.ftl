# Command
holopad-command-lo = Командування - LO
holopad-command-mysta = Командування - Міста
holopad-command-cj = Командування - CJ
holopad-command-evac = Командування - Евакуація

# Epistemics
holopad-epistemics-anomaly = Епістеміка - Аномалія
holopad-epistemics-artifact = Епістеміка - Артефакт
holopad-epistemics-robotics = Епістеміка - Робототехніка
holopad-epistemics-rnd = Епістеміка - Д&Р
holopad-epistemics-front = Епістеміка - Перед дверима
holopad-epistemics-breakroom = Епістеміка - Кімната відпочинку
holopad-epistemics-mantis = Епістеміка - Мантис
holopad-epistemics-oracle = Епістеміка - Оракул

# Logistics
holopad-logistics-front = Логістика - Перед дверима
holopad-logistics-cargo-bay = Логістика - Вантажний відсік
holodpad-logistics-mailroom = Логістика - Поштова кімната
holodpad-logistics-mailfront = Логістика - Перед поштою
holopad-logistics-salvage-bay = Логістика - Відсік шахтарів
holopad-logistics-breakroom = Логістика - Кімната відпочинку
holopad-logistics-ats = Логістика - Автоматична Торгова Станція
holopad-logistics-shuttle = Логістика - Шатл

# Justice
holopad-justice-prosecutor = Юстиція - Прокурор
holopad-justice-attorney = Юстиція - Адвокат
holopad-justice-clerk = Юстиція - Клерк

# Security
holopad-security-corpsman = Безпека - БрігМедик

# General
holopad-general-park = Загальний - Парк
holopad-general-nitrogen-lounge = Загальний - Нітрогеновий лаунж
holopad-general-shipyard-dock = Загальний - Верфовий док

# Medical
holopad-medical-psychologist = Медичний - Психолог
holopad-medical-outpost = Медичний - Форпост

# Engineering
holopad-engineering-pa-control = Інженерія - Контроль системи оголошень

# Service
holopad-service-toolroom = Сервіс - Кімната інструментів
holopad-service-barber-shop = Сервіс - Перукарня

# Station Specific
holopad-security-perma-kitchen = Безпека - Перма кухня
holopad-security-perma-botany = Безпека - Перма ботаніка
holopad-security-perma-yard-one = Безпека - Перма двір 1
holopad-security-perma-yard-two = Безпека - Перма двір 2
holopad-security-perma-workshop = Безпека - Перма майстерня
holopad-security-perma-guard-complex = Безпека - Перма комплекс охорони
holopad-security-perma-mineshaft = Безпека - Перма шахта
holopad-security-perma-arena = Безпека - арена
holopad-security-perma-gladiator-lounge = Безпека - лаунж гладіаторів
holopad-general-spesscar-track = Загальний - Спесскарський трек

